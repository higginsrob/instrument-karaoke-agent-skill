(function () {
  ...
  function drawFrame(canvas, state) {
    ...
    // Chords-and-lyrics chart scroll above drums (also used while aligning).
    const pad = 8;
    const rowGap = 4;
    ...
    const chartTop = pad;
    const chartH = Math.max(80, usableH - pad - drumsH - rowGap);
    ...
    const bandTop = drumsY;
    const totalH = drumsH;

    const dimDrums = Boolean(state.stemSilent?.drums);
    ctx.save();
    ctx.globalAlpha = dimDrums ? 0.35 : 1;
    drawDrums(ctx, state.drumsBeats, viewport, x, drumsY, w, drumsH);
    ctx.restore();

    drawPlayhead(ctx, pad, bandTop, w, totalH);
