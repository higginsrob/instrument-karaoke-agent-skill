(function () {
  ...
  const SHOW_DRUMS_STORAGE_KEY = "instrument-karaoke.show-drums-viz";

  function readShowDrumsPref() {
    try {
      const raw = localStorage.getItem(SHOW_DRUMS_STORAGE_KEY);
  ...
  function writeShowDrumsPref(on) {
    try {
      localStorage.setItem(SHOW_DRUMS_STORAGE_KEY, on ? "1" : "0");
    } catch {
      /* ignore quota / private mode */
    }
  }

  function hasDrumTrack(beats) {
    return Array.isArray(beats?.onsets) && beats.onsets.length > 0;
  }
  ...
  window.MediaPlayer = {
    ...
    async mount(container, { songPath, isAdmin, canExport, mediaStatus, chartBody, title, artist, onSyncState, onLyricEdit, onVerified }) {
      ...
      container.innerHTML = `
        ...
        <div class="media-audio" id="media-audio">
          <div class="media-audio-row">
            ...
            <label class="media-drums-toggle" id="media-drums-toggle" title="Show drum lane overlay">
              <input type="checkbox" id="media-show-drums" checked />
              <span>Drums</span>
            </label>
          </div>
      ...
      const drumsToggleWrap = container.querySelector("#media-drums-toggle");
      const drumsToggle = container.querySelector("#media-show-drums");
      ...
      let syncLines = [];
      let drumsBeats = null;
      /** User preference for drum lane overlay (ignored when song has no drum track). */
      let showDrumsPref = readShowDrumsPref();
      if (drumsToggle) drumsToggle.checked = showDrumsPref;
      let duration = 0;
      ...
      const api = {
        ...
        getMixState() {
          return {
            mix: exportMixId(mixId),
            showDrums: drumsVizEnabled(),
            hasDrumTrack: songHasDrumTrack(),
          };
        },
        setShowDrums(on) {
          if (!songHasDrumTrack()) {
          ...
          showDrumsPref = Boolean(on);
          writeShowDrumsPref(showDrumsPref);
          syncDrumsToggle();
          paint();
        },
      };
      ...
      function songHasDrumTrack() {
        return hasDrumTrack(drumsBeats);
      }

      function drumsVizEnabled() {
        return showDrumsPref && songHasDrumTrack();
      }

      function syncDrumsToggle() {
        if (!drumsToggle) return;
        const hasTrack = songHasDrumTrack();
        drumsToggle.disabled = !hasTrack;
        drumsToggle.checked = drumsVizEnabled();
        if (drumsToggleWrap) {
          drumsToggleWrap.classList.toggle("is-unavailable", !hasTrack);
          drumsToggleWrap.title = hasTrack
            ? "Show drum lane overlay"
            : "No drum track detected for this song";
        }
      }
      ...
      drumsToggle?.addEventListener("change", () => {
        ...
        showDrumsPref = Boolean(drumsToggle.checked);
        writeShowDrumsPref(showDrumsPref);
        syncDrumsToggle();
        paint();
      });
      ...
      container.querySelector("#media-export")?.addEventListener("click", () => {
        openExportModal({
          ...
          setShowDrums: (on) => api.setShowDrums(on),
          onJobUpdate(job) {
            setToolbarExportStatus(job);
            if (job && (job.status === "queued" || job.status === "processing")) {
  ...
  function openExportModal({ songPath, title, artist, chartBody, getMixState, setShowDrums, onJobUpdate }) {
    ...
    let selectedMix =
      state && typeof state.mix === "string" && MIX_MODES.some((m) => m.id === state.mix)
        ? state.mix
        : "original";
    const hasDrumTrack = Boolean(state?.hasDrumTrack);
    let showDrums = hasDrumTrack ? state?.showDrums !== false : false;
    ...
    overlay.innerHTML = `
      <div class="modal-dialog export-modal-dialog">
        ...
        <label class="export-show-drums${hasDrumTrack ? "" : " is-unavailable"}" id="export-show-drums-wrap" title="${hasDrumTrack ? "Include drum lane overlay" : "No drum track detected for this song"}">
          <input type="checkbox" id="export-show-drums"${showDrums ? " checked" : ""}${hasDrumTrack ? "" : " disabled"} />
          <span>Show drums</span>
        </label>
    ...
    const showDrumsInput = overlay.querySelector("#export-show-drums");
    showDrumsInput?.addEventListener("change", () => {
      if (!hasDrumTrack || jobId) {
        if (showDrumsInput) showDrumsInput.checked = showDrums;
        return;
      }
      showDrums = Boolean(showDrumsInput.checked);
      if (typeof setShowDrums === "function") setShowDrums(showDrums);
    });
