...
def sync_song_media_flags(
    library_root: Path,
    song_path: str,
    flags: dict[str, Any],
) -> bool:
    """Patch media flags for one song in index.json. Returns True if changed."""
    ...
    changed = False
    for song in songs:
        if not isinstance(song, dict) or song.get("path") != song_path:
            continue
        for key in keys:
            if key not in flags:
                continue
            if song.get(key) != flags[key]:
                song[key] = flags[key]
                changed = True
        break
    else:
