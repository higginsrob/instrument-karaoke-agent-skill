...
class OverlayRenderer:
    ...
    def __init__(self, width: int, height: int, *, lyric_display: str = "chart") -> None:
        ...
        self.drums_h = max(130, int(round(118 * s)))
        self.x = self.pad
        ...
        self.lane_h = self.drums_h / len(DRUM_LANES)
        ...
        self.chart_h = max(
            120,
            self.height - self.pad - self.drums_h - self.row_gap - self.progress_h,
        )
    ...
    def prepare(
        ...
        self.drums_silent = bool(drums_silent)
    ...
    def render_frame(self, *, current_time: float, duration: float) -> Image.Image:
        ...
        playhead_top = drums_y
        playhead_h = self.drums_h
        ...
        if self.drums_silent:
            drums_img.putalpha(drums_img.getchannel("A").point(lambda a: int(a * 0.35)))
        img.paste(drums_img, (int(x), int(drums_y)), drums_img)

        playhead_x = x + w * PLAYHEAD_RATIO
        draw.line(
            [(playhead_x, playhead_top), (playhead_x, playhead_top + playhead_h)],
